...
function Sidebar({ active, onChange, online, agentLevel, actor, profiles, onActorChange }) {
  return <aside className="sidebar">
    ...
    <nav><div className="nav-caption">工作区</div>{navItems.map((item) => <button key={item.id} className={active === item.id ? "active" : ""} onClick={() => onChange(item.id)}><span className="nav-icon">{item.icon}</span><span><strong>{item.label}</strong><small>{item.hint}</small></span></button>)}</nav>
