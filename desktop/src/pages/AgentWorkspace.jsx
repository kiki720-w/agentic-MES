...
export default function AgentWorkspace({ health, actor, onNavigate }) {
  ...
  return <div className="workspace-layout"><main className="conversation">
    ...
    <div className="quick-prompts"><button disabled={sending} onClick={selfCheck}>文字能力自检（3 项合成样本）<span>✓</span></button>{quickPrompts.map((item) => <button key={item} onClick={() => send(item)}>{item}<span>↗</span></button>)}</div>
