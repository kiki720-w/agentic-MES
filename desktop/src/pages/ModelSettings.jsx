...
import { api, formatDate } from "../platform";
...
const presets = {
  ...
  OLLAMA: { label: "Ollama 本地", baseUrl: "http://127.0.0.1:11434/v1", model: "qwen3:8b", key: false, note: "数据不出本机" },
...
export default function ModelSettings({ actor }) {
  ...
  return <div className="native-page models-page">
    ...
    <div className="model-status-banner"><div className="model-orbit">✦</div><div><span>当前推理模型</span><strong>{status.provider} · {status.model}</strong><p>{status.baseUrl}</p></div><StatusPill value={status.apiKeyConfigured || !currentPreset.key ? "已配置" : "未配置"} tone={status.apiKeyConfigured || !currentPreset.key ? "published" : "warning"} /></div>
    ...
    <Panel title="数据出厂策略" subtitle="正式版将由服务端网络策略强制执行，而不是依赖提示词。">
      ...
      <p className="status-footnote">配置来源：{status.configurationSource || "—"} · 最近检查：{formatDate(status.lastCheckedAt, true)} · {status.lastError || "无已知连接错误"}</p>
    </Panel>
  </div>;
}
