...
桌面 0.5.0 与 Core 已实现排产域第一条 L4 目标闭环：观察、计划、策略授权、幂等执行、回读核验、失败回滚和停止开关。排产可以发布到 CAPAXION 本地计划库；默认部署仍处于 SHADOW，试点环境可显式启用 `AUTONOMOUS + LOCAL_CORE`。能力缺口、缺料、质量冻结和输入版本变化仍会阻止执行。外部 MES 尚需专用写回适配器，因此当前声明为 `L4_RUNTIME_IMPLEMENTED_NOT_PRODUCTION_VALIDATED`。详见 [L4 排产运行时说明](docs/l4-scheduling-runtime.md)。

Agent 工作台是制造任务主入口：全窗口拖入、本机解析、结构化预检、授权写入和回读核验在一个对话中完成。左侧任务记录保存最多 50 个会话，每个会话根据首条任务和附件自动生成标题，并直接显示最近结果、任务状态、轮次、附件数量和时间；搜索、切换及删除均无需打开任务。空白会话会自动去重，已开始的对话不再重复显示欢迎语。切换业务页面不会卸载当前任务，桌面重启后也能继续查看。附件保存在本机内容寻址文档库中，并按问题检索带工作表、页码或章节来源的片段。人员能力表可以自动映射为产能资源，重复导入会按稳定人员编号更新而不会重复新增；车工计划表会优先读取“车工每日计划”，排除完成记录，并把未完成行转换为排产快照。自然语言新订单由当前模型转换为受校验的动作，随后创建工单并运行有限产能排产。产能、排产和结果页面保留人工检查与修改。详见 [工作台文档理解架构](docs/workbench-document-intelligence.md)、[本地附件拖放与理解](docs/local-attachment-understanding.md)和 [Agent 主导的本地生产闭环](docs/local-manufacturing-agent.md)。

早期自建 MES 流程仍保留为离线模拟器和回归测试夹具，不再作为正式产品入口或权威生产数据源。

## 本地 AI 与云端 MES 的独立网络边界（2026-09-13）

桌面 0.5.0 与 Core 默认使用本机 Qwen3 8B Q4_K_M；部署配置已可选择性授权 DeepSeek API，且不会自动回退到云端。工作台向模型携带当前会话最近 10 轮消息，并在本机保留完整任务记录。当前规则辅助制造基准通过 9/9，中位耗时 472 ms；历史 4B 原始模型的 6/9 结果继续保留，不能与系统任务路径混为一谈。该结果只适用于固定合成题，不等于生产准确率。详见 [准确率优化记录](docs/manufacturing-accuracy.md)和[网络边界说明](docs/independent-network-boundaries.md)。
