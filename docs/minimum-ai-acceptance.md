...
这些调用只发送合成数据。PDF/图片理解、完整多轮、生产准确率、成本和 LOCAL_ONLY 零出站尚未由本套三题验收。后续桌面 0.4.5 已接入本地 PDF 文字提取、图片/扫描 PDF OCR、Docling XLSX 结构识别、人员能力实际写入、最近 10 轮会话和自然语言工单排产；生产验收应分别统计字段提取、动作校验、写入一致性和排产可执行率，不能继续用文字题分数替代。
