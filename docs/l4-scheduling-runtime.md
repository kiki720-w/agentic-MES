...
本版发布验证结果：Ruff、Mypy 和 Vite 构建通过；Core 测试为 153 项通过、5 项因未配置隔离 PostgreSQL 测试库而跳过；打包后的七个桌面页面全部完成加载和滚动检查，没有前端运行时异常。0.4.0 还在打包版中完成了从非工作台页面拖入、自动跳转、本地解析、附件随问题发送和本地模型回答的端到端验证。便携安装包为 `desktop/release/CAPAXION-0.4.0-x64.exe`，SHA256 为 `45FF3A63D416B4E1373C5511D15C5138F164EF2E533322D4CAF59BFE69E47C8C`。
