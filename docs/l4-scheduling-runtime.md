...
本版发布验证结果：Ruff、Mypy 和 Vite 构建通过；Core 测试为 157 项通过、5 项因未配置隔离 PostgreSQL 测试库而跳过；打包后的七个桌面页面全部完成加载和滚动检查，没有前端运行时异常。0.4.3 在真实《车工计划.xlsx》拖入测试中识别 3 个工作表，并区分通用附件理解与标准 MES 导入预检。便携安装包为 `desktop/release/CAPAXION-0.4.3-x64.exe`，SHA256 为 `CBD381D97AC21A471872A3D5E989D6F269B6EC3ECA7D0C274E099D74A3B2604F`。
