...
本版发布验证结果：Ruff、Mypy 和 Vite 构建通过；Core 测试为 153 项通过、5 项因未配置隔离 PostgreSQL 测试库而跳过；打包后的七个桌面页面全部完成加载和滚动检查，没有前端运行时异常。0.4.1 还在打包版中验证了醒目的文件选择入口、版本标识和虚拟文件拖入后的本地解析。便携安装包为 `desktop/release/CAPAXION-0.4.1-x64.exe`，SHA256 为 `9A625ECDDCA2BEB5CD712FAB63C9664024F5E3937F1C39F896FA8F4F43B502F2`。
