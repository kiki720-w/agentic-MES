...
本版发布验证结果：Ruff、Mypy 和 Vite 构建通过；Core 测试为 153 项通过、5 项因未配置隔离 PostgreSQL 测试库而跳过；打包后的七个桌面页面全部完成加载和滚动检查，没有前端运行时异常。0.4.2 在真实 XLSX 二进制的打包版拖入测试中，同时通过了通用附件解析和带工厂查询参数的生产数据预检。便携安装包为 `desktop/release/CAPAXION-0.4.2-x64.exe`，SHA256 为 `86F0D3165BD1220FBC02C0E5173A3B34D0A9C32EF4129E3B38F8FEFCE6FF2195`。
